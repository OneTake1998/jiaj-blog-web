      ...
      <div class="search-container" v-if="openSearchContainer">
          ...
          <div>
            <section v-if="searchResults.length > 0">
              ...
              <ul id="search-menu">
                ...
                >
                  <a @click="handleLinkClick(result)">
                    <div class="search-hit-container">
                      ...
                      <div class="search-hit-content-wrapper">
                        <span
                          ...
                          v-html="result.title"
                        ></span>
                        <span class="search-hit-path">{{ result.content }}</span>
                      </div>
        ...
        <div class="search-footer">
          <div class="search-logo">
            ...
            >
              <span class="search-label">搜索引擎</span>
              ...
              <img
                class="mr-1.5  diamond"
                :src="avatar"
                alt="JiaJ Logo"
                height="20"
                width="20"
              />
              <span class="text-ob">{{nickname}}</span>
            </a>
          </div>
          <ul class="search-commands">
            <li>
              ...
              <span class="search-commands-label"> 查看 </span>
            </li>
...
export default {
  ...
  methods: {
    handleStatusChange(status) {
      this.$store.commit("search/SET_OPNEMODAL", status);
    },
    handleArrowUp() {
      if (this.isEmpty === true) return;
      if (this.menuActiveIndex === 0) {
        this.menuActiveIndex = this.menuMaxIndex;
      } else {
        this.menuActiveIndex = this.menuActiveIndex - 1;
      }
      // moveMenuWindow();
    },
    handleLinkClick(result) {
      this.$store.commit("search/SET_OPNEMODAL", false);
      this.$router.replace(`/article/${result.id}`);
    },
    handleArrowDown() {
      if (this.isEmpty === true) return;
      if (this.menuActiveIndex === 0) {
    ...
    recentResults() {},
    searchHandle() {
      this.$searchArticle({ title: this.keyword }).then((res) => {
        this.searchResults = res.data.records;
        this.searchResultsCount = res.data.records.length;
      });
    },
  ...
  computed: {
    openModal() {
      return this.$store.state.search.openModal;
    },
    avatar() {
  ...
  watch: {
    openModal(status) {
      // if (status === true) this.reloadRecentResult();
      this.$store.commit("search/SET_OPNEMODAL", status);
      setTimeout(() => {
        this.openSearchContainer = status;
      }, 200);
    },
  },
};
